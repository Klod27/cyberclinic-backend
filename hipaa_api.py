...
def normalize_answers_for_storage(answers):
    """
    Converts frontend answer objects into simple strings for database storage.
    """
    return {
...
def safe_org_id(raw_org_id):
    """
    Prevents database crashes when frontend sends 'default'.
    """
...
def build_findings(answers):
    ...
    for q in hipaa_questions:
        qid = q.get("id")
        answer = normalize_answer(answers.get(qid, "")).lower()
        ...
        if answer in ("no", "partial"):
            ...
            finding = {
                ...
                "description": f"{q.get('question')} was answered as {answer.title()}.",
                "impact": "Potential HIPAA compliance gap requiring review.",
                "business_impact": "May increase regulatory, operational, or patient trust risk.",
                "recommendation": f"Review and implement control: {q.get('question')}"
            }
...
# ----------------------------------
# SUBMIT ASSESSMENT — PUBLIC DEMO MODE
# ----------------------------------
...
def submit_assessment(
    ...
    try:
        ...
        org_id = safe_org_id(data.get("org_id"))
        ...
        # ----------------------------------
        findings, remediation = build_findings(answers)
        ...
        try:
            ...
            print("AI recommendation error:", str(ai_error))
            ai_recommendations = [
        ...
        # ----------------------------------
        # SAVE ASSESSMENT IF DATABASE ALLOWS
        # ----------------------------------
        ...
        try:
            ...
            simple_answers = normalize_answers_for_storage(answers)

            for q_id, answer_value in simple_answers.items():
                db.add(HIPAAAnswer(
                    assessment_id=assessment.id,
                    question_id=q_id,
                    answer=answer_value
            ...
            db.rollback()
            print("Database save skipped:", str(db_error))

        # ----------------------------------
        # RESPONSE FOR FRONTEND + DASHBOARD
        # ----------------------------------
        report_data = {
...
def get_results(
    ...
    findings, remediation = build_findings(answers_dict)
